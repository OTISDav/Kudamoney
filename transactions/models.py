...
class Transaction(models.Model):
    """
    Modèle représentant une transaction financière entre deux utilisateurs.
    """
    ...
    final_amount = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Montant final après réduction", null=True, blank=True) # Nouveau champ
    discount_code_used = models.ForeignKey('DiscountCode', on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Code de réduction utilisé") # Nouveau champ
    status = models.CharField(max_length=20, choices=TRANSACTION_STATUS, default='pending', verbose_name="Statut")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Date de création")

    def __str__(self):
        return f"Transaction de {self.sender.username} à {self.receiver.username} - {self.status}"
